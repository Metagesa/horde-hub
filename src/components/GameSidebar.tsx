...
export function GameSidebar() {
  ...
  return (
    <>
      ...
      >
        <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_top,rgba(180,28,36,0.2),transparent_26%),linear-gradient(180deg,transparent,rgba(0,0,0,0.2))]" />
            ...
            {isLoading ? (
              <div className="space-y-3 px-1">
                {[1, 2, 3].map((item) => (
                  <div
                    key={item}
                    className="site-panel h-[78px] rounded-[24px] border border-white/8 bg-white/[0.03] animate-pulse"
                  />
                ))}
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div
                        className={`h-10 w-1 rounded-full transition-colors ${
                          active ? "bg-gold-light" : "bg-transparent group-hover:bg-red-400/40"
                        }`}
                      />
                    </Link>
                  );
          ...
          <div className="border-t border-sidebar-border/80 px-4 py-4">
            <div className="site-panel rounded-[22px] border border-red-500/20 bg-red-950/10 px-4 py-3 text-center">
              <p className="text-xs text-muted-foreground">
                Hecho por Metagesa ❤ para la Horda de Plata
              </p>
            </div>
          </div>
        </div>
